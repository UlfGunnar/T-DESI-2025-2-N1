...
class UsuarioDAO:
    ...
    def Atualizar(self,novo_nome, id_usuario):
        cursor = self.conexao.cursor()
...
class TestUsuarioDAO(unittest.TestCase):
    ...
    def test_salvar_e_buscar_objeto(self):
        ...
        usuario_atualizado = self.dao.Atualizar("Ana Carolina", usuario_salvo.id)
        usuario_recuperado = self.dao.buscar_por_id(usuario_salvo.id)
        self.assertEqual(usuario_recuperado.nome, "Ana Carolina")

        usuario_deletado = self.dao.Deletar(usuario_salvo.id)
